...
def get_filters():
    ...
    while True:
        day = input('\nWhich day of the week would you like to filter by? Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday? Type "all" if you do not have any preference.\n').lower()
...
def station_stats(df):
    ...
    combi_station = df.groupby(['Start Station', 'End Station']).count()
    ...
    while True:
        ...
        if raw_data not in ('yes', 'no'):
           print("Sorry! I did not catch that. Please try again!")
            ...
            # Display lines rows_start untill rows_end
            print ('\n',df.iloc[rows_start : rows_end + 1])
            rows_start += show_rows
            rows_end += show_rows

            print('\nWould you like to see the next {} lines?\n'.format(show_rows))
            continue
